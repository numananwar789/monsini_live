...
@section('page-js')
    ...
    <script>
        $(document).ready(function() {
            // make cost a float right away (changed)
            ...
            // ------- updated pricing logic (only function changed) -------
            function updatePrice() {
                ...
                let addition = 0;
                if (sizeProd >= 18) {
                    // Bridal styles: start with "B" => +60; otherwise +30
                    addition = (style.charAt(0).toUpperCase() === 'B') ? 60 : 30;
                }

                const total = quantity * (base + addition);
                const formatted = (total % 1 === 0) ? total : total.toFixed(2); // clean display (no .00)
                $('#price').val(formatted);
            }
            // -------------------------------------------------------------
            ...
            $('#style').on('change', updatePrice); // added to keep price live on style change
        });
    </script>
@endsection
