...
//         //         MAX(dt_product.product_color) as product_color,
//         //         MAX(dt_product.product_size_range) as product_size_range,
//         //         MAX(dt_product.product_cost) as product_cost,
//         //         MAX(dt_product.product_wholesale_price) as product_wholesale_price,
//         //         MAX(dt_product.product_vendor_ID) as product_vendor_ID,
...
//         //         SUM(dt_inventory.product_quantity) AS inventory_count
//         //     ')
//         //     ->where('dt_product.product_vendor_ID', '!=', 0)
//         //     ->where('dt_product.product_status', 1)
//         //     ->groupBy('dt_product.product_style')
//         //     ->orderByRaw('Max(dt_product.product_style) ASC, MAX(dt_product.product_color) ASC')
//         //     ->get();
...
//         -- 🔥 FIXED INVENTORY COUNT (IMPORTANT)
//         CASE
//             WHEN MAX(dt_product.product_status) = 1 
//                 THEN 1   -- Active → always available

//             WHEN MAX(dt_product.product_status) = 0 
//                  AND MAX(dt_product.show_from_inventory) = 1 
//                 THEN SUM(dt_inventory.product_quantity)
...
//             AND MAX(dt_product.show_from_inventory) = 1
//             AND SUM(dt_inventory.product_quantity) > 0
//         )
//     ')
...
//     }
    ...
    public function index()
    {
        ...
        $products = Product::leftJoin('dt_inventory', 'dt_product.product_ID', '=', 'dt_inventory.product_ID')
            ->selectRaw('
                ...
                -- COALESCE(SUM(dt_inventory.product_quantity), 0) as inventory_count,
    ...
    //     $on_way = DB::table('dt_order_allocation')
    //         ->select(
    //             'order_product_style',
    //             'order_product_color',
    //             'order_product_size',
    ...
    //         ->selectRaw('MAX(product_status) as product_status, MAX(show_from_inventory) as show_from_inventory')
    //         ->first();
    
    //     $productStatus = $productMeta->product_status ?? 0;
    //     $showFromInventory = $productMeta->show_from_inventory ?? 0;
    
    //     $onWayQuery = DB::table('dt_order_allocation')
    //         ->select('order_product_style', 'order_product_color', 'order_product_size', 'order_quantity')
    //         ->where('order_product_style', $id)
    //         ->where('order_customer_name', $ownerComp)
    //         ->where('order_quantity', '>', 0);
    
    //     // CASE: Inactive + Show From Inventory
    //     if ($productStatus == 0 && $showFromInventory == 1) {
    
    //         $stock = DB::table('dt_inventory')
    //             ->select('product_style', 'product_color', 'product_size', DB::raw('SUM(product_quantity) as final_quantity'))
    //             ->where('product_style', $id)
    //             ->groupBy('product_style', 'product_color', 'product_size')
    //             ->havingRaw('SUM(product_quantity) > 0')
    //             ->get();
    
    //         $sizes  = collect($stock)->pluck('product_size')->unique()->values();
    //         $colors = collect($stock)->pluck('product_color')->unique()->values();
    
    //         return response()->json([
    //             'mode'     => 'inventory',
    //             'products' => [],
    //             'stock'    => $stock,
    //             'on_way'   => $onWayQuery->get(),
    //             'sizes'    => $sizes,
    //             'colors'   => $colors,
    //         ]);
    //     }
